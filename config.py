...
def log_entry(log_type, service_id, name, status):
    ...
    log_filename = os.path.join(log_folder, f'logfile_{current_date}_{service_id}.txt')
    
    # Check if the file already exists to write the header only once
    file_exists = os.path.isfile(log_filename)
    
    # Create the log entry
    log_entry = f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, {log_type}, {service_id}, {name}, {status}\n"
